...
void list_add_ascending_by_address(list_t *l, block_t *newblk){
   ...
   */
    ...
    // If the list is empty, the new block becomes the first node
    if (l->head == NULL) {
        l->head = new_node;
        return;
    }
    ...
    // Find the correct position where current->blk->start >= newblk->start
    ...
    if (previous == NULL) {
        // If the new block should be inserted at the beginning
        new_node->next = l->head;
        l->head = new_node;
    } else {
        // Otherwise, insert the new block after the previous node
...
void list_add_ascending_by_blocksize(list_t *l, block_t *newblk){
   ...
   *    USE the compareSize()
   */
    // Create a new node for new block
    node_t *new_node = (node_t *)malloc(sizeof(node_t));
    new_node->blk = newblk;
    new_node->next = NULL;
    ...
    // find block size
    int new_block_size = newblk->end - newblk->start + 1;

    // Traverse the list to find the correct position
    while (current != NULL) {
        int current_block_size = current->blk->end - current->blk->start + 1;

        // Use compareSize to decide where to insert
        if (new_block_size <= current_block_size) {
            // Insert the new node before the current node
            if (previous == NULL) {
                // Insert at the beginning if previous is NULL (i.e., head)
                new_node->next = l->head;
                l->head = new_node;
            } else {
    ...
    // If we reach the end of the list, add the new node to the end
    previous->next = new_node;
}
...
void list_coalese_nodes(list_t *l){ 
   ...
   */
    ...
    if (l->head == NULL || l->head->next == NULL) {
        return; // No coalescing needed
    }

    // Initialize two pointers for traversal: prev and current
    node_t *prev = l->head;
    ...
    while (current != NULL) {
        ...
        if (prev->blk->end + 1 == current->blk->start) {
            // They are adjacent, so coalesce them
            prev->blk->end = current->blk->end; // Merge the two blocks by extending the previous block's end

            // Skip over the current node by updating prev->next
            prev->next = current->next;

            // Free the memory for the current node as it's now merged
            free(current);

            // Move current to the next node (no need to advance prev since it remains the same after merge)
            current = prev->next;
        } else {
            // If they are not adjacent, move to the next pair of nodes
            prev = current;
            current = current->next;
        }
    }
   ...
   * Look at list_is_in_by_size()*/
   // Check if the list is empty
    if (l == NULL || l->head == NULL) {
        return false;  // Empty list, no blocks to check
    }
    ...
    while (current != NULL) {
        // Use comparePid to check if the current block's pid matches the target pid
